...
def func_query(time_data, limit, offset):
    query = gql('''
                query {
                  transactions(orderBy: timestamp, orderDirection:desc, where:{timestamp_not:null, 
                  timestamp_lt:'''+str(time_data)+'''}, first:'''+str(limit)+''', skip:'''+str(offset)+''') {
                    id
                    buyer {
                        id
                    }
...
def API_request_Godsunchained(limit, time_data, time_start, lines_to_save_data, data_folder):
    counter = 0
    df = pd.DataFrame({}, dtype=str)
    sample_transport=RequestsHTTPTransport(
        url='https://api.thegraph.com/subgraphs/name/djrosenbaum/unchained-marketplace',
        verify=True,
        retries=3,
    )
    client = Client(
    ...
    while(time_data>time_start):
        ...
        if len(response['transactions'])>0:#response.status_code==200:
            df_supp = pd.DataFrame(response['transactions'], dtype=str)#pd.DataFrame(response.json()['orders'], dtype=str)
            if len(df_supp)==0: break#repetitive
            df = df.append(df_supp, ignore_index=True)
            
            time_data_supp = int(df_supp.timestamp.min())#int(df_supp.created_at_time.min())
            ...
            if counter%lines_to_save_data==0:
                ...
                df.to_csv(data_folder+'NFT_Godsunchained_'+str(supp.month)+'_'+str(supp.year)+'.csv.gz', index=False)
        else: break
        time.sleep(1)
...
for i in range(len(dt_time)-1):
    data_folder = './Data_Godsunchained/'
    os.system('mkdir '+data_folder)
    data_folder += str(dt_time[-2-i].month)+'_'+str(dt_time[-2-i].year)+'/'
    print(data_folder)
    os.system('mkdir '+data_folder)
    API_request_Godsunchained(limit, dt_time[-1-i], dt_time[-2-i], lines_to_save_data, data_folder)
